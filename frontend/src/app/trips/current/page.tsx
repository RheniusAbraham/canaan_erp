...
const CURRENT_STATUSES: Trip["status"][] = [
  "Assigned",
  "Started",
  "Loaded",
  "On-Transit",
...
const FILTERS: Array<Trip["status"] | "All"> = ["All", ...CURRENT_STATUSES];

export default function CurrentTripsPage() {
  const [filter, setFilter] = useState<Trip["status"] | "All">("All");

  const currentTrips = initialTrips.filter(
    (trip) => CURRENT_STATUSES.includes(trip.status)
  );

  const trips =
    filter === "All"
      ? currentTrips
      : currentTrips.filter((trip) => trip.status === filter);
